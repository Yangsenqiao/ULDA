...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: torch.Tensor, use_relu=True):
        identity = x
...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, trunc1 = False, trunc2 = False, trunc3=False,trunc4=False, get1=False, get2=False,get3=False, get4=False, use_relu=True):
        def stem(x):
            ...
            x = self.avgpool(x)
            return x
        if not trunc1 and not trunc2 and not trunc3 and not trunc4 and  use_relu:
            x = x.type(self.conv1.weight.dtype)
            x = stem(x)
            x = self.layer1(x)
            if get1:
                return x
        if not trunc1 and not trunc2 and not trunc3 and not trunc4 and not use_relu and get1:
            x = x.type(self.conv1.weight.dtype)
            x = stem(x)
            for layer in self.layer1[:2]:
                x = layer(x)
            return self.layer1[2](x,use_relu=False)
